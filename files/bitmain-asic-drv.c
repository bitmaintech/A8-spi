...
void check_asic_status(BT_AS_INFO dev)
{
	...
	if ((dev->cgminer_start == true) && (gTotal_asic_num !=0))
    {
		...
		if (gNonce_num > gTotal_asic_num * 8)
        {
			...
			bgNonce_average = gNonce_num / gTotal_asic_num / 8;
            for (i = 0; i < CHAIN_SIZE; i++)
            {
                for (j = 0; j < gChain_Asic_num[i]; j++)
                {
                    if (gAsic_cnt[i][j] < bgNonce_average)
                    {
...
void send_to_pfga_work(struct work_struct *work)
{
	...
	bool over_temp_work = false;
	#ifdef send_new_bl_CTRL
	bool have_new_bl = false;
	#endif
	...
	*/
		...
		(can_send_work == true)/*&& (snd_to_fpga_work < 1)*/)
	{
		...
		/*
		if((dev->asic_configure.snd_work_when_temp_h == false) && (dev->temp_out_fool == true))
			break;
		*/
		if(dev->new_block)
		{
		...
		{
			#if 0
			S1_timeout = jiffies - 10 * HZ/1000;
			no_nonce_timeout = true;
			dev->temp_out_fool = true;
			#endif
			check_status_timeout = S1_timeout;
		}
...
no_send:
	...
	if(time_after(jiffies, check_status_timeout))
	{
		...
		{
			printk(KERN_ERR "temp_highest{%d}\n", dev->temp_highest);
		}
	}
	/*
	if(time_after(jiffies, M30_timeout))
	{
		M30_timeout = jiffies + 30 * 60 * 1000 * HZ/1000;//30 minute
		printk("total nonce num = %lu, snd_to_fpga_work{%d}\n", ret_nonce_num, snd_to_fpga_work);
		for (i = 0; i < CHAIN_SIZE; i++)
        {
			...
			for (j = 0; j < gChain_Asic_num[i]; j++)
			{
				if((j % 16 ) == 0)
					printk("\n");
				printk("as%02d[%08lu], ", j, gAsic_cnt[i][j]);
			}
			printk("\n");
		}
	}
	*/
	if(((dev->temp_out_fool == true)&&(dev->all_fan_stop == true)) || (no_nonce_timeout == true))
	{
		if((dev->asic_configure.beep_on_en) && (dev->beep_status == false))
		{
			dev->beep_ctrl = true;
		}
	}
